...
export default function EurekaDevPage() {
  ...
  return (
    <div className="relative min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] overflow-x-hidden">
	      {/* Header */}
	      <header className="fixed top-6 left-6 md:w-auto md:right-auto right-6 z-40 border border-[color:var(--border-subtle)] backdrop-blur-md bg-[var(--nav-bg)] rounded-[16px]">
	        <div className="w-full mx-auto px-6">
	          <div className="flex items-center gap-6 md:h-14 h-14">
	            ...
	            <nav className="hidden md:flex items-center gap-8">
	              <button onClick={() => scrollToSection("about")} className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors duration-300">About</button>
	              <button onClick={() => scrollToSection("tracks")} className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors duration-300">Tracks</button>
	              <button onClick={() => scrollToSection("schedule")} className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors duration-300">Schedule</button>
	              <button onClick={() => scrollToSection("faq")} className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors duration-300">FAQ</button>
	              <a href="https://discord.gg/wECtZ3csKb" target="_blank" rel="noopener noreferrer" className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors duration-300">Discord</a>
	              <a href="https://eurekadev.devpost.com/" target="_blank" rel="noopener noreferrer" className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors duration-300">Devpost</a>
	            </nav>

	            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="md:hidden ml-auto p-2 hover:bg-[var(--chip-bg)] rounded-lg transition-colors duration-300" aria-label="Toggle menu">
      ...
      {/* Mobile Menu */}
	      {isMenuOpen && (
	        <div className="fixed inset-0 bg-[var(--menu-bg)] backdrop-blur-md z-50 flex flex-col items-start justify-end pb-20 pt-20 px-6">
	          <button onClick={() => setIsMenuOpen(false)} className="absolute top-6 right-6 p-2 hover:bg-[var(--chip-bg)] rounded-lg transition-colors duration-300" aria-label="Close menu">
	            <X className="w-6 h-6" />
	          </button>
	          <div className="flex flex-col gap-8 items-start text-left w-full">
	            <button onClick={() => scrollToSection("about")} className="font-serif text-5xl md:text-7xl font-light text-[var(--text-primary)] hover:text-[color:var(--accent-blue)] transition-colors duration-300">About</button>
	            <button onClick={() => scrollToSection("tracks")} className="font-serif text-5xl md:text-7xl font-light text-[var(--text-primary)] hover:text-[color:var(--accent-blue)] transition-colors duration-300">Tracks</button>
	            <button onClick={() => scrollToSection("schedule")} className="font-serif text-5xl md:text-7xl font-light text-[var(--text-primary)] hover:text-[color:var(--accent-blue)] transition-colors duration-300">Schedule</button>
	            <button onClick={() => scrollToSection("faq")} className="font-serif text-5xl md:text-7xl font-light text-[var(--text-primary)] hover:text-[color:var(--accent-blue)] transition-colors duration-300">FAQ</button>
	            <a href="https://discord.gg/wECtZ3csKb" target="_blank" rel="noopener noreferrer" className="font-serif text-5xl md:text-7xl font-light text-[var(--text-primary)] hover:text-[color:var(--accent-blue)] transition-colors duration-300">Discord</a>
	            <a href="https://eurekadev.devpost.com/" target="_blank" rel="noopener noreferrer" className="font-serif text-5xl md:text-7xl font-light text-[var(--text-primary)] hover:text-[color:var(--accent-blue)] transition-colors duration-300">Devpost</a>
	          </div>
	        </div>
	      )}

	      {/* Hero Section */}
	      <section ref={heroRef} className={`relative min-h-screen flex flex-col items-center justify-center px-4 pt-24 pb-16 md:pt-32 md:pb-24 transition-all duration-[1200ms] ease-[cubic-bezier(0.16,1,0.3,1)] overflow-hidden ${isLoaded ? "scale-100 opacity-100" : "scale-[1.03] opacity-0"}`} style={{ backgroundImage: `url('/bg.png')`, backgroundSize: "cover", backgroundPosition: "center", backgroundAttachment: "fixed" }}>
	        <div className="absolute inset-0 pointer-events-none" style={{ transform: `translateY(${scrollY * 0.5}px)`, backgroundImage: `url('/bg.png')`, backgroundSize: "cover", backgroundPosition: "center" }} />
          ...
          <div className="text-center mb-8 md:mb-12">
            ...
            <h1 className="font-serif text-[44px] leading-[1.1] md:text-[72px] md:leading-[1.05] font-medium mb-6 text-balance">
              ...
              <span className="block stagger-reveal text-5xl font-light md:text-7xl" style={{ animationDelay: "90ms" }}>
                into <span className="text-[color:var(--accent-blue)]">impact</span>
              </span>
            </h1>
	            <p className="text-[var(--text-secondary)] text-base md:text-lg max-w-[600px] mx-auto mb-8 leading-relaxed stagger-reveal" style={{ animationDelay: "180ms" }}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center stagger-reveal" style={{ animationDelay: "270ms" }}>
              <a href="https://eurekadev.devpost.com/" target="_blank" rel="noopener noreferrer">
                <Button className="px-8 py-6 text-base rounded-full bg-[color:var(--accent-blue)] hover:opacity-90 text-[color:var(--on-accent-blue)] font-medium transition-opacity duration-200">Register on Devpost</Button>
              </a>
              <a href="https://discord.gg/wECtZ3csKb" target="_blank" rel="noopener noreferrer">
                <Button className="px-8 py-6 text-base rounded-full bg-transparent border border-[color:var(--border-strong)] hover:bg-[var(--chip-bg)] hover:border-[color:var(--border-strong)] transition-all duration-200 text-[var(--text-primary)]">Join Discord</Button>
              </a>
	              <Button onClick={() => scrollToSection("about")} className="px-8 py-6 text-base rounded-full bg-transparent border border-[color:var(--border-strong)] hover:bg-[var(--chip-bg)] hover:border-[color:var(--border-strong)] transition-all duration-200 text-[var(--text-primary)]">Learn More</Button>
            </div>
          </div>
          ...
          <h2 className="font-serif text-[32px] leading-[1.15] md:text-[48px] md:leading-[1.1] font-medium mb-6 md:mb-8 text-center text-balance">
            Innovation <span className="text-[color:var(--accent-blue)]">Without Limits</span>
	          ...
	          <p className="text-[var(--text-secondary)] text-sm md:text-base mb-12 md:mb-16 text-center max-w-[700px] mx-auto leading-relaxed">
		            EurekaDev combines the spirit of a science fair with the creativity of a hackathon. Whether you want to code, research, or both - this is your chance to innovate across multiple categories and share your vision with a global community.
	          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 max-w-[900px] mx-auto">
            {[
              ...
              { label: "TRACKS", value: "2", desc: "Coding & Research", color: "var(--accent-violet)" },
		              ...
		              <div
		                key={i}
		                className="p-6 md:p-8 text-center bg-[var(--surface-1)] rounded-2xl"
		              >
	                ...
	                <div className="text-[11px] md:text-xs text-[var(--text-secondary)] mt-3">{metric.desc}</div>
	              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-[1120px] w-full mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <div className="order-2 lg:order-1">
	              <div className="text-[10px] md:text-xs uppercase tracking-[0.15em] text-[color:var(--accent-blue)] mb-4 flex items-center gap-2">
	                <Code className="w-4 h-4" />
	                CODING TRACK
	              </div>
              <h3 className="font-serif text-[28px] leading-[1.15] md:text-[40px] md:leading-[1.1] font-medium mb-6 text-balance">
	                Build real solutions with <span className="text-[color:var(--accent-blue)]">code</span>
              </h3>
            ...
            <div className="order-1 lg:order-2">
	              <div className="relative rounded-2xl overflow-hidden">
	                <Image
	                  src="https://upload.wikimedia.org/wikipedia/commons/5/5a/Programming.jpg"
        ...
        <div className="max-w-[1120px] w-full mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <div>
	              <div className="relative rounded-2xl overflow-hidden">
	                <Image
	                  src="https://upload.wikimedia.org/wikipedia/commons/0/09/Cahier_Atoma_ouvert.jpg"
            ...
            <div>
	              <div className="text-[10px] md:text-xs uppercase tracking-[0.15em] text-[color:var(--accent-orange)] mb-4 flex items-center gap-2">
	                <FileText className="w-4 h-4" />
	                RESEARCH TRACK
	              </div>
              <h3 className="font-serif text-[28px] leading-[1.15] md:text-[40px] md:leading-[1.1] font-medium mb-6 text-balance">
	                Explore ideas through <span className="text-[color:var(--accent-orange)]">research</span>
              </h3>
	              <p className="text-[var(--text-secondary)] text-sm md:text-base mb-8 leading-relaxed">
      ...
      <section className="relative py-20 md:py-32 px-4 animate-on-scroll">
        <div className="max-w-[1120px] w-full mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <div className="order-2 lg:order-1">
	              <div className="text-[10px] md:text-xs uppercase tracking-[0.15em] text-[color:var(--accent-violet)] mb-4 flex items-center gap-2">
	                <Star className="w-4 h-4" />
	                WORKSHOPS & MENTORSHIP
	              </div>
              <h3 className="font-serif text-[28px] leading-[1.15] md:text-[40px] md:leading-[1.1] font-medium mb-6 text-balance">
	                Learn from <span className="text-[color:var(--accent-violet)]">industry experts</span>
              </h3>
	              <p className="text-[var(--text-secondary)] text-sm md:text-base mb-8 leading-relaxed">
            ...
            <div className="order-1 lg:order-2">
	              <div className="relative rounded-2xl overflow-hidden">
	                <Image
	                  src="https://upload.wikimedia.org/wikipedia/commons/9/90/Physics-informed_nerural_networks.png"
      ...
      <section className="relative py-20 md:py-32 px-4 animate-on-scroll">
        <div className="max-w-[1120px] w-full mx-auto">
          <div className="text-center mb-12 md:mb-16">
	            <div className="text-[10px] md:text-xs uppercase tracking-[0.15em] text-[var(--text-secondary)] mb-6 flex items-center justify-center gap-2">
	                            GLOBAL COMMUNITY
	            </div>
      ...
      <section id="tracks" className="relative py-20 md:py-32 px-4 animate-on-scroll">
        <div className="max-w-[1120px] w-full mx-auto">
          <div className="text-center mb-12 md:mb-16">
            ...
            <h2 className="font-serif text-[32px] leading-[1.15] md:text-[48px] md:leading-[1.1] font-medium mb-6 text-balance">
              Choose Your <span className="text-[color:var(--accent-blue)]">Track</span>
            </h2>
	            <p className="text-[var(--text-secondary)] text-sm md:text-base max-w-[600px] mx-auto leading-relaxed">We offer two tracks so participants can showcase their strengths</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
	            {[{ title: "Coding Track", icon: Code, desc: "Build a functional prototype, app, website, simulation, or other coded solution. Perfect for developers and aspiring engineers.", items: ["Apps & websites", "AI models", "Simulations", "Technical solutions"] }, { title: "Research Track", icon: FileText, desc: "Conduct background research and create a literature review, theoretical framework, or experimental design around your chosen problem.", items: ["Literature reviews", "Experimental designs", "Theoretical frameworks", "Data analysis"] }].map((track, i) => (
		              <div key={i} className={`rounded-2xl p-8 md:p-10 transition-all duration-200 ${selectedTrack === i ? "bg-[var(--surface-2)] shadow-md" : "bg-[var(--surface-1)]"}`}>
		                <track.icon
		                ...
		                <p className="text-[var(--text-secondary)] text-sm md:text-base leading-relaxed mb-6">{track.desc}</p>
	                <ul className="space-y-2">
	                  {track.items.map((item, j) => (<li key={j} className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">{item}</li>))}
	                </ul>
	              </div>
	            ))}
          </div>
          ...
          <div className="text-center mb-8">
            <h3 className="text-xl md:text-2xl font-medium mb-4">Subject Categories</h3>
	            <p className="text-[var(--text-secondary)] text-sm md:text-base max-w-[500px] mx-auto">Within each track, participants can choose from 6 categories</p>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {categories.map((cat, i) => {
	              const catColor = i % 3 === 0 ? "var(--accent-blue)" : i % 3 === 1 ? "var(--accent-orange)" : "var(--accent-violet)"
	              return (
		              <div key={i} className="bg-[var(--surface-1)] rounded-xl p-4 md:p-6 flex items-center gap-3">
      ...
      <section className="relative py-20 md:py-32 px-4 animate-on-scroll">
        <div className="max-w-[1120px] w-full mx-auto">
          <div className="text-center mb-12 md:mb-16">
            ...
            <h2 className="font-serif text-[32px] leading-[1.15] md:text-[48px] md:leading-[1.1] font-medium mb-6 text-balance">
              Prizes for <span className="text-[color:var(--accent-orange)]">all participants</span>
            </h2>
	            <p className="text-[var(--text-secondary)] text-sm md:text-base max-w-[600px] mx-auto leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { icon: Trophy, title: "Grand Prize", desc: "Scholarships to prestigious research programs and exclusive mentorship opportunities", color: "var(--accent-blue)" },
              { icon: Star, title: "Track Winners", desc: "Category-specific prizes for top performers in Coding and Research tracks", color: "var(--accent-orange)" },
              { icon: Rocket, title: "All Participants", desc: "Certificates, exclusive swag, and networking opportunities for everyone who submits", color: "var(--accent-violet)" },
            ].map((prize, i) => (
		              <div key={i} className="bg-[var(--surface-1)] rounded-2xl p-8 text-center">
		                <prize.icon className="w-10 h-10 mx-auto mb-6" style={{ color: prize.color }} />
		                <h3 className="text-xl font-medium mb-3">{prize.title}</h3>
		                <p className="text-[var(--text-secondary)] text-sm leading-relaxed">{prize.desc}</p>
		              </div>
            ))}
          </div>
        </div>
      ...
      {/* Schedule Section */}
	      <section id="schedule" className="relative py-20 md:py-32 px-4 animate-on-scroll bg-[var(--bg-primary)]">
            ...
            <h2 className="font-serif text-[32px] leading-[1.15] md:text-[48px] md:leading-[1.1] font-medium mb-6 text-balance">
              Program <span className="text-[color:var(--accent-orange)]">Schedule</span>
            </h2>
          </div>
          ...
          <div className="space-y-6">
            {[{ week: "Week 1", title: "Opening Ceremony & Ideation", items: ["Opening Ceremony", "Mission & Goals", "Design Thinking & Idea Selection", "Subject Workshops (CS, AI, Medical)"] }, { week: "Week 2", title: "Workshops & Team Development", items: ["Continued workshops", "Team development", "Mentorship sessions"] }, { week: "Week 3", title: "Presentation Prep", items: ["How to Pitch Workshop", "Presentation Prep Workshop", "Project feedback sessions"] }, { week: "Week 4", title: "Solution Development", items: ["Asynchronous work time", "Office hours", "One-on-one mentoring"] }, { week: "Week 5", title: "Submissions & Awards", items: ["Project Submissions (max 4-min video)", "Judging", "Award & Closing Ceremony", "Guest Speaker Panel"] }].map((week, i) => (
		              <div key={i} className="bg-[var(--surface-1)] rounded-2xl p-6 md:p-8">
		                <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-8">
		                  <div className="flex-shrink-0">
		                    <span
		                      className="inline-block px-4 py-2 rounded-full border text-[color:var(--accent-orange)] text-sm font-medium"
		                      style={{
		                        borderColor:
	                    ...
	                    <ul className="flex flex-wrap gap-2">{week.items.map((item, j) => (<li key={j} className="text-xs md:text-sm text-[var(--text-secondary)] bg-[var(--chip-bg)] border border-[color:var(--chip-border)] px-3 py-1 rounded-full">{item}</li>))}</ul>
	                  </div>
	                </div>
	              </div>
            ))}
	            ...
	            {[{ question: "Who can participate in EurekaDev?", answer: "EurekaDev is open to high school students aged 13-19 from all countries worldwide. You can compete individually or with a team of up to 4 teammates." }, { question: "Do I need coding experience to participate?", answer: "Not at all! We offer two tracks - a Coding Track for those who want to build technical solutions, and a Research Track for those who prefer literature reviews, theoretical frameworks, or experimental designs. We also provide workshops throughout the competition to help beginners learn." }, { question: "What do I need to submit?", answer: "You need to submit a project video (max 4 minutes), a written project description, and either code documentation (for Coding Track with a GitHub repo link) or a research PDF (for Research Track). Live demos are recommended but not required for coding projects." }, { question: "Can I use AI tools in my project?", answer: "AI use for writing code/research documents is allowed but discouraged. AI use for ideation and support is encouraged. Quality is a judging criteria, so make sure your work demonstrates genuine understanding and effort." }, { question: "Are projects created before the hackathon permitted?", answer: "Yes, projects created before the event timeframe are permitted, however they will receive a point deduction during judging." }, { question: "How do I join and stay updated?", answer: "Register on our Devpost page, then join our Discord server for announcements, workshops, and updates — joining the Discord is required to be eligible for prizes. You can also follow us on Instagram. For any questions, email us at eureka.institute.contact@gmail.com or ask in the Questions channel on Discord." }].map((faq, i) => (
		              <div key={i} className="bg-[var(--surface-1)] rounded-xl overflow-hidden">
		                <button onClick={() => setOpenFaqIndex(openFaqIndex === i ? null : i)} className="w-full flex items-center justify-between p-6 text-left">
		                  ...
		                  <ChevronDown className={`w-5 h-5 flex-shrink-0 text-[var(--text-secondary)] transition-transform duration-300 ${openFaqIndex === i ? "rotate-180" : ""}`} />
		                </button>
		                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${openFaqIndex === i ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"}`}>
		                  <p className="px-6 pb-6 text-sm md:text-base text-[var(--text-secondary)] leading-relaxed">{faq.answer}</p>
		                </div>
		              </div>
            ))}
          </div>
      ...
      <section id="cta" className="relative py-24 md:py-40 px-4 animate-on-scroll overflow-hidden bg-[var(--bg-primary)]">
        <div className="max-w-[800px] w-full mx-auto text-center relative z-10">
          ...
          <p className="text-[var(--text-secondary)] text-base md:text-lg mb-10 leading-relaxed max-w-[560px] mx-auto">Code, research, and win prizes. Join hundreds of students from around the world in EurekaDev, September 27 – October 25.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="https://eurekadev.devpost.com/" target="_blank" rel="noopener noreferrer">
              <Button className="text-base rounded-full bg-[color:var(--accent-blue)] hover:opacity-90 text-[color:var(--on-accent-blue)] font-medium transition-opacity duration-200 px-8 py-6">Register on Devpost</Button>
            </a>
            <a href="https://discord.gg/wECtZ3csKb" target="_blank" rel="noopener noreferrer">
              <Button className="text-base rounded-full bg-transparent border border-[color:var(--border-strong)] hover:bg-[var(--chip-bg)] hover:border-[color:var(--border-strong)] transition-all duration-200 text-[var(--text-primary)] px-8 py-6">Join Discord Server</Button>
            </a>
          </div>
        </div>
      </section>
      ...
      {/* Footer */}
      <footer className="relative px-4 border-t border-[color:var(--border-subtle)] py-8">
        <div className="max-w-[1120px] w-full mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-8 mb-12">
            <div className="flex flex-col gap-4">
              <div className="text-lg font-semibold font-mono">EurekaDev</div>
              ...
              <div className="mt-4 space-y-4">
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <a href="https://www.eurekainstitute.xyz/" target="_blank" rel="noopener noreferrer" className="text-[var(--text-secondary)] hover:text-[color:var(--accent-blue)] transition-colors" aria-label="Eureka Institute Website">
                ...
                <div>
                  <div className="text-[10px] uppercase tracking-[0.15em] text-[var(--text-secondary)] mb-2">EurekaDev</div>
                  <div className="flex items-center gap-4">
                    <a href="https://discord.gg/wECtZ3csKb" target="_blank" rel="noopener noreferrer" className="text-[var(--text-secondary)] hover:text-[color:var(--accent-blue)] transition-colors" aria-label="EurekaDev Discord">
            ...
            <div className="flex flex-col gap-4">
              <div className="text-xs uppercase tracking-[0.15em] text-[var(--text-primary)] font-semibold mb-2">Quick Links</div>
              <div className="flex flex-col gap-3">
                <button onClick={() => scrollToSection("about")} className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors text-left">About</button>
                <button onClick={() => scrollToSection("tracks")} className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors text-left">Tracks</button>
                <button onClick={() => scrollToSection("schedule")} className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors text-left">Schedule</button>
                <button onClick={() => scrollToSection("faq")} className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors text-left">FAQ</button>
              </div>
            </div>

            <div className="flex flex-col gap-4">
              <div className="text-xs uppercase tracking-[0.15em] text-[var(--text-primary)] font-semibold mb-2">Resources</div>
              <div className="flex flex-col gap-3">
                <a href="https://eurekadev.devpost.com/" target="_blank" rel="noopener noreferrer" className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">Devpost Page</a>
                <a href="https://www.eurekainstitute.xyz/" target="_blank" rel="noopener noreferrer" className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">Eureka Institute</a>
                <a href="https://discord.gg/wECtZ3csKb" target="_blank" rel="noopener noreferrer" className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">Discord Server</a>
                <a href="https://docs.google.com/document/d/1Y7UJF9GO88y6imqEjcE2kyPyv6QwieMmWFFdpY8GVAM/edit?usp=sharing" target="_blank" rel="noopener noreferrer" className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">Research Template</a>
              </div>
            </div>

            <div className="flex flex-col gap-4">
              <div className="text-xs uppercase tracking-[0.15em] text-[var(--text-primary)] font-semibold mb-2">Contact</div>
              <p className="text-xs text-[var(--text-secondary)] mb-3">Questions? Reach out to us!</p>
              <a href="mailto:eureka.institute.contact@gmail.com" className="flex items-center gap-2 text-sm text-[var(--text-secondary)] hover:text-[color:var(--accent-blue)] transition-colors"><Mail className="w-4 h-4" />eureka.institute.contact@gmail.com</a>
            </div>
          </div>
